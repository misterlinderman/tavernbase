...
import { listCaptainMatches, submitScoresheet } from '../../services/captain';
import type {
  CaptainMatch,
...
function PoolScoresheetForm({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!Number.isInteger(payload.homeRaceWins) || payload.homeRaceWins < 0) {
      toast('Enter a valid home team race wins', 'error');
      return;
    }

    if (!Number.isInteger(payload.awayRaceWins) || payload.awayRaceWins < 0) {
      toast('Enter a valid away team race wins', 'error');
      return;
    }
    ...
    try {
      await submitScoresheet(captainFetch, match._id, payload);
...
function DartsScoresheetForm({
  ...
  const ourLabel = match.mySide === 'home' ? match.homeTeamName : match.awayTeamName;
  const theirLabel = match.mySide === 'home' ? match.awayTeamName : match.homeTeamName;
  ...
  return (
    <form className={styles.scoreForm} onSubmit={handleSubmit}>
      ...
      <p className={styles.matchMeta}>
        Enter home and away legs won ({ourLabel} vs {theirLabel}).
      </p>
      <div style={{ gridColumn: '1 / -1' }}>
        <button type="submit" className="btn btn-green" disabled={submitting}>
          {submitting ? 'Submitting…' : 'Submit scoresheet'}
...
function VolleyballScoresheetForm({
  ...
  return (
    <form className={styles.scoreForm} onSubmit={handleSubmit}>
      <p className={styles.matchMeta}>
        {formatLabel} — first team to {setsToWin} sets wins the match.
      </p>
      <div>
...
function CaptainPage() {
  const { captainFetchList } = useCaptainApi();
  const { toast } = useToast();
  const [matches, setMatches] = useState<CaptainMatch[]>([]);
  const [loading, setLoading] = useState(true);

  const loadMatches = useCallback(async () => {
    const data = await listCaptainMatches(captainFetchList, 'open');
    setMatches(data);
  }, [captainFetchList]);
  ...
  return (
    <div>
      <h1 className={formStyles.pageTitle}>Captain Scoresheets</h1>
      <p className={styles.matchMeta}>
        Submit match results for your team. Both captains must enter matching scores before the
        result is final.
      </p>
      ...
      {matches.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          {matches.map((match) => {
            ...
            const mySummary = mySheet ? formatScoresheetSummary(mySheet.payload, sport) : null;
            const opponentSummary = opponentSheet
              ? formatScoresheetSummary(opponentSheet.payload, sport)
              : null;
            ...
            return (
              <article key={match._id} className={styles.matchCard}>
                <div className={styles.matchHeader}>
                  ...
                  <span className={`${styles.submissionState} ${stateClassName(match.submissionState)}`}>
                ...
                {mySheet?.status === 'submitted' || mySheet?.status === 'approved' ? (
                  <p className={styles.scoreReadout}>
                    Your submission: {mySummary?.home}–{mySummary?.away} (home–away {mySummary?.unit})
                  </p>
                ) : null}

                {opponentSheet?.status === 'submitted' && match.canSubmit && opponentSummary ? (
                  <div className={styles.opponentScore}>
                    Opponent submitted {opponentSummary.home}–{opponentSummary.away}. Enter the same
                    score to confirm.
                  </div>
                ) : null}
                ...
                {match.canSubmit ? (
                  <MatchScoreForm match={match} onSubmitted={loadMatches} />
                ) : null}
              </article>
            );
          })}
